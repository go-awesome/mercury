...
func LoadP12(filename, password string) (*x509.Certificate, *rsa.PrivateKey, error) {
	p12, err := ioutil.ReadFile(filename)
	if err != nil {
		return nil, nil, fmt.Errorf("Unable to load %s: %v", filename, err)
	}
	return decodeP12(p12, password)
}

// Decode and verify an in memory .p12 certificate (DER binary format).
func decodeP12(p12 []byte, password string) (*x509.Certificate, *rsa.PrivateKey, error) {
...
func TLSCertificate(cert *x509.Certificate, privateKey *rsa.PrivateKey) tls.Certificate {
